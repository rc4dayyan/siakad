...
<head>
    ...
    <title>Jadwal Kuliah {{ $student->mhs_nim }} — {{ $period?->name }}</title>
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .print-toolbar { position: sticky; z-index: 20; top: 0; display: flex; justify-content: flex-end; gap: 8px; padding: 12px max(18px, calc((100vw - 1100px) / 2)); border-bottom: 1px solid #d6dfdc; background: rgba(255, 255, 255, .96); box-shadow: 0 4px 16px rgba(11, 33, 53, .08); }
        ...
        .sheet { width: min(1100px, calc(100% - 32px)); min-height: 760px; margin: 22px auto; padding: 28px; background: #fff; box-shadow: 0 16px 45px rgba(11, 33, 53, .14); }
        .letterhead { display: grid; grid-template-columns: 76px 1fr 185px; align-items: center; gap: 16px; padding-bottom: 14px; border-bottom: 3px solid var(--navy); }
        ...
        th, td { border-right: 1px solid #afbfba; border-bottom: 1px solid #afbfba; padding: 7px 6px; vertical-align: middle; }
        ...
        .day { display: inline-block; min-width: 48px; margin-bottom: 4px; padding: 3px 6px; border-radius: 5px; color: #0b5f51; background: var(--green-soft); font-size: 7px; font-weight: 800; text-align: center; text-transform: uppercase; }
...
<body>
    ...
    <main class="sheet">
        <header class="letterhead">
            ...
            <div class="document-label"><small>Dokumen Mahasiswa</small><strong>Jadwal Kuliah</strong></div>
        </header>

        <section class="title">
            <span>{{ $period?->name ?? 'Periode belum tersedia' }}</span>
            <h2>Jadwal Kuliah Mahasiswa</h2>
            <p>Daftar pertemuan perkuliahan pada periode akademik berjalan</p>
        </section>
        ...
        <table>
            <thead><tr><th class="number">No.</th><th class="date">Tanggal &amp; Waktu</th><th class="course">Mata Kuliah</th><th class="lecturer">Nama Dosen</th><th>Kelas &amp; Lokasi</th><th class="meeting">Pertemuan</th></tr></thead>
            <tbody>
                @forelse ($schedules as $schedule)
                    <tr>
                        <td class="number">{{ $loop->iteration }}</td>
                        <td class="date"><span class="day">{{ $schedule->days_id }}</span><strong>{{ \Carbon\Carbon::parse($schedule->date)->locale('id')->translatedFormat('d M Y') }}</strong><small>{{ substr($schedule->start, 0, 5) }}–{{ substr($schedule->ended, 0, 5) }} WIB</small></td>
                        <td class="course"><strong>{{ $schedule->matkul?->name ?? 'Mata kuliah tidak tersedia' }}</strong><small>{{ $schedule->matkul?->code ?? $schedule->code }} · {{ $schedule->bsks }} SKS</small></td>
                        ...
                        <td class="meeting"><strong>{{ $schedule->pert_id }}</strong><small>{{ $schedule->meth_id }}</small></td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="empty">Tidak ada jadwal kuliah yang sesuai dengan filter.</td></tr>
                @endforelse
            </tbody>
        </table>

        <div class="note"><strong>Keterangan:</strong> Dokumen ini menampilkan jadwal yang berhak diakses mahasiswa dan mengikuti filter pada halaman Jadwal Kuliah. Perubahan jadwal setelah dokumen dicetak mengikuti pembaruan pada SIAKAD.</div>
