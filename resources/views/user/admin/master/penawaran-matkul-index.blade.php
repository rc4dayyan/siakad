...
<section class="section">
    ...
    <div class="modal fade" id="createOfferingModal" tabindex="-1" aria-labelledby="createOfferingModalLabel" aria-hidden="true"><div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable"><div class="modal-content">
        ...
        <div class="modal-body">@if($errors->any() && old('_form') === 'create-offering')<div class="alert alert-danger"><ul class="mb-0">@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div>@endif<form id="create-offering-form" method="POST" action="{{ route($prefix.'master.penawaran-store') }}" class="row g-3">@csrf<input type="hidden" name="_form" value="create-offering">
            <div class="col-md-4"><label class="form-label">Master mata kuliah</label><select name="master_mata_kuliah_id" class="form-select" required>@foreach($masters as $item)<option value="{{ $item->id }}" @selected(old('master_mata_kuliah_id') == $item->id)>{{ $item->name }} ({{ $item->sks }} SKS)</option>@endforeach</select></div>
            <div class="col-md-4"><label class="form-label">Program studi</label><select name="pstudi_id" class="form-select" required>@foreach($programs as $item)<option value="{{ $item->id }}" @selected(old('pstudi_id') == $item->id)>{{ $item->name }}</option>@endforeach</select></div>
            <div class="col-md-4"><label class="form-label">Kurikulum</label><select name="kuri_id" class="form-select" required>@foreach($curricula as $item)<option value="{{ $item->id }}" @selected(old('kuri_id') == $item->id)>{{ $item->name }}</option>@endforeach</select></div>
            ...
            <div class="col-md-4"><label class="form-label">Prasyarat</label><select name="prasyarat_master_id" class="form-select"><option value="">Tanpa prasyarat</option>@foreach($masters as $item)<option value="{{ $item->id }}" @selected(old('prasyarat_master_id') == $item->id)>{{ $item->name }}</option>@endforeach</select></div>
    ...
    @endif
    <div class="card"><div class="card-header d-flex justify-content-between align-items-center"><h5 class="card-title">Daftar Penawaran — {{ $period?->name ?? 'Belum ada periode' }}</h5>@if($canManage)<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createOfferingModal"><i class="fas fa-plus me-1"></i> Tambah Penawaran</button>@endif</div><div class="card-body table-responsive">
