...
class HomeController extends Controller
{
    ...
    public function galleryIndex()
    {
        ...
        $data['albums'] = GalleryAlbum::where('isPublish', 1)->latest()->paginate(24);
        $data['prefix'] = $this->setPrefix();
        $data['title'] = ' - Siakad';
    ...
    public function gallerySearch(Request $request)
    {
        ...
        $albums = GalleryAlbum::where('isPublish', 1)
    ...
    public function galleryShow($slug)
    {
        ...
        $data['notify'] = Notification::whereIn('send_to', [0, 3])->get();
        $data['web'] = webSettings::where('id', 1)->first();
        $data['album'] = GalleryAlbum::where('slug', $slug)
            ->where('isPublish', 1)
            ->firstOrFail();
        $data['albums'] = GalleryAlbum::where('isPublish', 1)
            ->whereKeyNot($data['album']->getKey())
            ->latest()
            ->take(3)
            ->get();
        $data['prefix'] = $this->setPrefix();
